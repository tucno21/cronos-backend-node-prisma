...
const fields = {
    ...
    userId: check('userId', 'El usuario es obligatorio').not().isEmpty(),
    etiquetas: check('etiquetas', 'La etiqueta es obligatoria').not().isEmpty(),
}

router.get('/', getBlogs);
router.get('/:id', getBlog);
router.post('/', [fields.titulo, fields.contenido, fields.userId, fields.etiquetas, validarCampos], createBlog);
